  ...
  Future<Either<String, List<PlanetDto>>> getPlanetData() async {
    ...
    try {
      final QuerySnapshot<Map<String, dynamic>> res =
          await _firestore.collection('planets').get();
  ...
  Future<Either<String, PlanetDto>> getPlanetDetailsByID(
    ...
    try {
      final DocumentSnapshot res =
          await _firestore.collection('planets').doc(id).get();
  ...
  Future<List<String>> addToBookmark({required String id}) async {
    ...
    try {
      ...
      await _firestore
          .collection('users')
          .doc(uid)
          .update({'bookmarks': updatedBookmarks}).then(
              (value) => updatedBookmarkList = updatedBookmarks);
    } catch (e) {
      return [];
    }
